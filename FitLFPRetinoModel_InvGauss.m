function [finalParameters,fisherInfo,ninetyfiveErrors] = FitLFPRetinoModel_InvGauss(Response,xaxis,yaxis,centerVals)
%FitLFPRetinoModel_InvGauss.m
...
%    120 msec after stimulus presentation, assumes an 
...
Bounds = [0,600;min(xaxis),max(xaxis);min(yaxis),max(yaxis);1,1000;1,1000;1,5000;0,1000];
...
%     display(sprintf('Running Data for Channel %d...',zz));
    ...
    parfor repeats = 1:numRepeats
        ...
        proposal = [1.75,100;4,250;4,250;1.75,150;1.75,150;1.5,500;1.75,200];
...
for kk=1:reps
    for jj=1:numParameters
       ...
       likelihood = 0.5*log(tempParams(6)/(2*pi*peakNegativity(kk)^3))-...
        (tempParams(6)*(peakNegativity(kk)-mu)^2)/(2*(mu^2)*peakNegativity(kk));
       Jacobian(kk,jj) = (likelihood-prevLikely(kk))/h(jj);
    end
    tempParams = parameterVec+h;
    mu = tempParams(1)*exp(-((flashPoints(kk,1)-tempParams(2)).^2)./(2*tempParams(4).^2)-...
        ((flashPoints(kk,2)-tempParams(3)).^2)./(2*tempParams(5).^2))+tempParams(7);
    tempLikely(kk) = 0.5*log(tempParams(6)/(2*pi*peakNegativity(kk)^3))-...
        (tempParams(6)*(peakNegativity(kk)-mu)^2)/(2*(mu^2)*peakNegativity(kk));
end
end
...
for kk=1:reps
    ...
    loglikelihood(kk) = 0.5*log(parameterVec(6)/(2*pi*peakNegativity(kk)^3))-...
        (parameterVec(6)*(peakNegativity(kk)-mu)^2)/(2*(mu^2)*peakNegativity(kk));
end
